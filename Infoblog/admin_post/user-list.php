...
<section class="vh-100" style="background-color: #2779e2;">
  <div class="container h-100">
    ...
    $limit = 4; // how many record you want to show
     ...
     $offset = ($page - 1) * $limit;
     $sql = "SELECT user.user_no,user.username,user.user_type FROM user
     ORDER BY user.user_no DESC  LIMIT {$offset},{$limit}";
    ...
    ?>
        ...
        <tbody class="table-group-divider">
          ...
          <td class='id'><?php echo $row['user_no'];?></td>
            ...
            <td>
            <a href="delete_user.php?id=<?php echo $row['user_no'];?>">
    ...
    if (mysqli_num_rows($result1) > 0) {
        ...
        if ($page > 1) {
            echo '<li class="page-item"><a class="page-link" href="user-list.php?page=' . ($page - 1) . '">Previous</a></li>';
        }
        for ($i = 1; $i <= $total_page; $i++) { // for each page
            // for li color after color
            $active = ($i == $page) ? "active" : "";
            echo '<li class="page-item ' . $active . '"><a class="page-link" href="user-list.php?page=' . $i . '">' . $i . '</a></li>';
        }
        if ($total_page > $page) {
            echo '<li class="page-item"><a class="page-link" href="user-list.php?page=' . ($page + 1) . '">Next</a></li>';
        }
